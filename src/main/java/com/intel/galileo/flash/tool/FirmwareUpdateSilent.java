...
public class FirmwareUpdateSilent {
   ...
   private String _findGalileoSerialPort()  // this is the function I will add to the silent class, it only needs to open an instance of the cmdargs class
   {
	String port = null;
	 
	GalileoVersion objFirmwareVer;
	String         strFirmwareVer;
	List<String> ports = _cmdArgs.queryPorts();
	String serialport = null;

	for(int j=0; j<ports.size(); j++)
	{
		// Give the user serial port to the flasher
		_galileoflash.setCommunicationConnection(ports.get(j));
		...
		{   
			port = ports.get(j);
			_cmdArgs.setUserPort(port);
			break;
		}
            }
            // if the first enumerated port is not a Galileo Stop the update
            catch (Exception ex)
            {

                Logger.getLogger(FirmwareUpdateTool.class.getName())
                         .log(java.util.logging.Level.SEVERE, null, ex);
                System.out.println("Error: " + ports.get(j) + " is valid system serial port but it is connected to a Galileo.");
                ...
                System.exit(FirmwareUpdateTool.E_BADCOMPORT);
   ...
   private boolean _setSerialPort()
   {
     ...
     catch (Exception ex)
     {
         ...
         Logger.getLogger(FirmwareUpdateTool.class.getName())
                  .log(java.util.logging.Level.SEVERE, null, ex);
         System.out.println("Error: " + _cmdArgs.getSerialPort() + " is valid system serial port but it is connected to a Galileo.");
         System.out.println("If you did not specify this port, then the first enumerated port is not connected to Galileo.");
         System.out.println("Please specify the correct serial port connected to a Galileo after the -serial_port option.");
   ...
   public void runSilent(boolean aAllowFirmwareFlash)
    {
      ...
      // if no port is specified then scan for a Galileo
      if (_cmdArgs.getSerialPort().contentEquals("NULL")) {
         if (_findGalileoSerialPort() == null) {
             _systemExitCode = FirmwareUpdateTool.E_NOBOARD;
